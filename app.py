...
def extrair_pesquisa_com_claude(arquivo_bytes, mime_type, api_key):
    ...
    prompt = f"""Analise este documento de pesquisa eleitoral para o Senado do Amazonas 2026.

Candidatos relevantes:
- Alberto Neto (PL) — "Capitão Alberto Neto"
- Eduardo Braga (MDB)
- Plinio Valerio (PSDB) — "Plínio Valério"
- Wilson Lima (União Brasil)
- Marcos Rotta (Avante / PDT)
- Marcelo Ramos (PT)

INSTRUÇÕES:
1. Se tiver 1º voto E 2º voto SEPARADOS: calcule Opção B:
   soma = 1v + 2v para cada candidato
   optB[c] = soma[c] / total_de_todas_somas * 100
2. Se tiver só um número por candidato: use diretamente.
3. Use 0.0 para candidatos não testados.
...
{{
  ...
  "Marcos Rotta": 5.0,
  "Marcelo Ramos": 8.0,
  "observacao": "nota metodológica breve"
}}"""
